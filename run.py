...
def my_cash_converter():
    while True:
        ...
        try:
            ...
            if "error" in data:
                print(f"Conversion from {from_currency} to {to_currency}"
                      f"is not supported.")
            else:
                # Extract conversion rate from the response.
                conversion_rate = data["conversion_rate"]

                # Performs conversion and gives user the amount he inputs.
                converted_amount = amount * conversion_rate
                print(f"{amount} {from_currency} is equal to"
        ...
        # If user choice is not yes.Break out of the loop to end the program.
        if choice != "yes":
            break
